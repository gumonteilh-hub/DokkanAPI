...
function extractCharacterData(characterDocument: Document) {
  ...
  const characterData: Character = {
    ...
    maxLevel: parseInt(
      (characterDocument.querySelector(".mw-parser-output")?.querySelector("table > tbody > tr:nth-child(3) > td")?.textContent?.split("/")[1] ||
        characterDocument.querySelector(".mw-parser-output")?.querySelector("table > tbody > tr:nth-child(3) > td")?.textContent?.split("/")[0]) ??
        "Error"
    ),
    ...
    activeSkill:
      (characterDocument.querySelector('[data-image-name="Active skill.png"]')?.closest("tr")?.nextElementSibling?.textContent ||
        characterDocument.querySelector('[data-image-name="Active skill.png"]')?.closest("tr")?.nextElementSibling?.nextElementSibling?.textContent) ??
      undefined,
    activeSkillCondition:
      characterDocument.querySelector('[data-image-name="Active skill.png"]')?.closest("tr")?.nextElementSibling?.nextElementSibling?.nextElementSibling?.querySelector("td > center")?.textContent ??
      undefined,
    ...
    ezaActiveSkillCondition:
      characterDocument
        ...
        ?.closest("tr")
        ?.nextElementSibling?.nextElementSibling?.nextElementSibling?.querySelector("td > center")?.textContent ?? undefined,
    transformationCondition:
      characterDocument.querySelector('[data-image-name="Transformation Condition.png"]')?.closest("tr")?.nextElementSibling?.querySelector("td > center")?.textContent ?? undefined,
    links: Array.from(characterDocument.querySelector('[data-image-name="Link skill.png"]')?.closest("tr")?.nextElementSibling?.querySelectorAll("span > a") ?? []).map(
    ...
    kiMeter: Array.from(characterDocument.querySelector('[data-image-name="Ki meter.png"]')?.closest("tbody")?.querySelectorAll("img") ?? [])
      .map((kiMeter) => kiMeter.getAttribute("alt")?.split(".png")[0] ?? "Error")
      .slice(1),
    ...
    maxLevelHP: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(3) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    freeDupeHP: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(4) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    ...
    baseAttack: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(2) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    maxLevelAttack: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(3) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    freeDupeAttack: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(4) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    rainbowAttack: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(5) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    baseDefence: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(4) > td:nth-child(2) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    maxDefence: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(4) > td:nth-child(3) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    freeDupeDefence: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(4) > td:nth-child(4) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    rainbowDefence: parseInt(
      characterDocument.querySelector(".righttablecard > table:nth-child(3) > tbody:nth-child(1) > tr:nth-child(4) > td:nth-child(5) > center:nth-child(1)")?.textContent ?? "Error"
    ),
    kiMultiplier:
      characterDocument
        .querySelector(".righttablecard > table:nth-child(6) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1)")
        ?.innerHTML.split("► ")[1]
        .split("<br>")[0]
        .concat("; ", characterDocument.querySelector(".righttablecard > table:nth-child(6) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1)")?.innerHTML.split("<br>► ")[1] ?? "")
        .replace('<a href="/wiki/Super_Attack_Multipliers" title="Super Attack Multipliers">SA Multiplier</a>', "SA Multiplier") ??
      characterDocument.querySelector(".righttablecard")?.nextElementSibling?.querySelector("tr:nth-child(2) > td")?.textContent?.split("► ")[1] ??
      "Error",
...
function extractTransformedCharacterData(characterDocument: Document): Transformation[] {
  ...
  for (let index = 1; index < transformCount; index++) {
    const transformationData: Transformation = {
      transformedName:
        characterDocument
          .querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2}) > table > tbody > tr > td:nth-child(2)`)
      ...
      transformedClass:
        Classes[
          (characterDocument
            .querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2}) > table > tbody > tr:nth-child(3) > td:nth-child(4) > center > a`)
            ?.getAttribute("title")
            ?.split(" ")[0]
            .split("Category:")[1] as keyof typeof Classes) ?? "error"
        ],
      ...
      transformedSuperAttack:
        characterDocument
          .querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2})`)
          ?.querySelector('[data-image-name="Super atk.png"]')
          ?.closest("tr")?.nextElementSibling?.textContent ?? "Error",
      transformedEZASuperAttack:
        characterDocument
          .querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2})`)
          ?.querySelector(".righttablecard > table > tbody > tr > td > div > div > div:nth-child(3)")
          ?.querySelector('[data-image-name="Super atk.png"]')
      ...
      transformedImageURL:
        characterDocument.querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2}) > table > tbody > tr > td > div > img`)?.getAttribute("src") ??
        characterDocument.querySelector(`.mw-parser-output > div:nth-child(2) > div:nth-child(${index + 2}) > table > tbody > tr > td > a`)?.getAttribute("href") ??
        "error",
    };
