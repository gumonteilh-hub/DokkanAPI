...
export function cleanData(liteData: Card[]): Card[] {
  ...
  const filteredData = sortedData.filter((card) => {
    ...
    for (const character of sortedData) {
      ...
      if (charId < evolvedId) return true;
    }
    return true;
  });
...
export function parseLeaders(leaderAsString: string): LeaderRecap {
  ...
  if (secondaryLeaderMatch) {
    ...
    secondaryLeaders = secondaryLeaderMatch
      .slice(2, 5)
      .filter(Boolean)
      .map((category) => ({
        category: parseCategory(category),
